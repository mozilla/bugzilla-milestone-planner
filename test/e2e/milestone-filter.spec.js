...
test.describe('Milestone Filter', () => {
  test('should filter milestone cards when selecting Customer Pilot', async ({ page }) => {
    ...
    // Select Customer Pilot from dropdown
    console.log('Selecting Customer Pilot...');
    await page.selectOption('#milestone-filter', '2012055');
    ...
    // Verify only 1 card (Customer Pilot)
    expect(afterCards).toBe(1);
    expect(cardContent[0]).toContain('Customer Pilot');
  });
});
